...
dataset = sys.argv[1]
if dataset == "opp":
    path = os.path.join(os.path.expanduser('~'), 'Documents', 'pythonProject', 'opportunity.h5')
elif dataset =="dap":
    path = os.path.join(os.path.expanduser('~'), 'Documents', 'pythonProject','baseline-cnn-daphnet', 'daphnet.h5')
elif dataset =="pa2":
    path = os.path.join(os.path.expanduser('~'), 'Documents', 'pythonProject', 'pamap2.h5')
elif dataset =="sph":
...
with tf.Session() as session:
    # laod network graph
    techer = tf.train.import_meta_graph('model/teacher.ckpt.meta')

    tf.initialize_all_variables().run()
    techer.restore(session, tf.train.latest_checkpoint('model'))
...
#            summary_writer.add_summary(summary,global_step.eval(session=session))
    ...
    plt.title("Loss value over epochs (teacher-student model)")
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
    plt.show()
